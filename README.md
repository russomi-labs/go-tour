...
``` Go
// Check it out here: https://tonnygaric.com/blog/newtons-method-in-go
...
func Sqrt(x float64) float64 {
	...
	for {
		// Value with previous output of z
		//
		// Check later if z has changed since last iteration
		previousOutput := z

		// Adjust z based on how close z^2 is to x
		z -= (z*z - x) / (2 * x)

		// Stop loop condition once z has not changed since last iteration
		if z == previousOutput {
			fmt.Println("z stopped changing after", i, "iterations.\n")
			break
		}
...
func main() {
	// Declare this floating point value by using conversion
	//
	// Also possible:
	// var x float64 = 2
	//
	// or using floating point syntax:
	// x := 2.0
	x := float64(2)

	fmt.Println("Sqrt output for x =", x, "is", Sqrt(x))
	fmt.Println("math.Sqrt output for x =", x, "is", math.Sqrt(x), "\n")
